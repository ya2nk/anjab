		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">Form Master Jabatan</h4>
				</div>
...
<script>
	$(function(){
		table = $('#myTable').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": "<?= site_url('master_jabatan/data'); ?>",
				"type":"POST",
				"data": function ( d ) {
					d.myKey = "myValue";
	...
	{
		$('.modal-body').html('');
		$.post('<?= site_url('master_jabatan/form'); ?>',{id:id}).done(function(result){
	...
	{
		$_confirm(function(){
			$.post('<?= site_url('master_jabatan/delete'); ?>',{id:id},function(result){
				if (result == 'success'){
