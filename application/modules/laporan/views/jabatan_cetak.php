 ...
 <center><h4>INFORMASI JABATAN</h4></center>
	<table class="table">
					...
					<tr>
						...
						<tr>
						<th width="4%">4. </th>
						<th>Kedudukan Dalam Struktur Organisasi</th>
						<td>:</td>
					...
					<tr>
						...
						<td>
							<table>
								...
								<?php if (isset($tugas_pokok)) : foreach ($tugas_pokok as $k=>$r) :  ?>
								<tr>
					...
					<tr>
						...
						<td>
							<table class="table table-bordered">
								...
								<?php $perangkat = json_decode($row->perangkat_kerja); if (count($perangkat) >0) : foreach($perangkat as $k=>$pr) : ?>
								<tr>
									<td><?= $k+1; ?></td>
					...
					<tr>
						...
						<td>
							<table>
								...
								<?php $tanggung_jawab = json_decode($row->tanggung_jawab); if (count($tanggung_jawab) >0) : foreach($tanggung_jawab as $k=>$tj) : ?>
								<tr>
									<td><?= $abjad[$k]; ?>. </td>
					...
					<tr>
						...
						<td>
							<table>
								...
								<?php $wewenang = json_decode($row->wewenang); if (count($wewenang) >0) : foreach($wewenang as $k=>$w) : ?>
								<tr>
									<td><?= $abjad[$k]; ?>. </td>
									<td><?= $w->wewenang; ?></td>
					...
					<tr>
						...
						<td>
							<table class="table table-bordered">
								...
								<?php $korelasi = json_decode($row->korelasi_jabatan); if (count($korelasi) >0) : foreach($korelasi as $k=>$ko) : if ($ko->korelasi_jabatan == '') continue; ?>
								<tr>
									<td><?= $k+1; ?></td>
					...
					<tr>
						...
						<td>
							<table class="table table-bordered">
								...
								<?php $resiko = json_decode($row->resiko_berbahaya); if (count($resiko) >0) : foreach($resiko as $k=>$re) : ?>
								<tr>
									<td><?= $k+1; ?></td>
