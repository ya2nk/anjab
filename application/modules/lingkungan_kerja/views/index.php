		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">Form Master Lingkungan Kerja</h4>
				</div>
...
<script>
	$(function(){
		table = $('#myTable').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": "<?= site_url('lingkungan_kerja/data'); ?>",
				"type":"POST",
				"data": function ( d ) {
					d.myKey = "myValue";
	...
	{
		$('.modal-body').html('');
		$.post('<?= site_url('lingkungan_kerja/form'); ?>',{id:id}).done(function(result){
	...
	{
		$_confirm(function(){
			$.post('<?= site_url('lingkungan_kerja/delete'); ?>',{id:id},function(result){
				if (result == 'success'){
